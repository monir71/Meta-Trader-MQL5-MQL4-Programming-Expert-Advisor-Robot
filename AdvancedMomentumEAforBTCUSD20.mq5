...
void TradeNowOnMomentum()
{
   ...
   while(Check_Momentum_Entry() == "Buy" && GetPositions_Buy() < 2 && GetTripleMovingAverageSignal() == "Buy Now!")
   {
      //For H1
      ...
      //For Bitcoin only
      trade.Buy(0.01, NULL, Ask, 0, Ask + 10000 * _Point, NULL);
   }
   
   while(Check_Momentum_Entry() == "Sell" && GetPositions_Sell() < 2 && GetTripleMovingAverageSignal() == "Sell Now!")
   {
      //For H1
      ...
      //For Bitcoin only
      trade.Sell(0.01, NULL, Bid, 0, Bid - 10000 * _Point, NULL);
   }
}
